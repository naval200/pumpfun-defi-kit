...
OPS_JSON="$DEBUG_DIR/amm-airdrop-1-5-no-ata.json"

echo "🚀 Script 2: AMM Batch airdrops to users 1-5 without ATA creation"
MINT=$(jq -r '.mint' "$TOKEN_INFO_JSON")

# Generate operations from template
echo "📋 Generating AMM airdrop operations from template..."
"$DEBUG_DIR/generate-operations.sh" amm-airdrop "$MINT"
