...
async function main() {
    try {
        ...
        if (args.help) {
            console.log('Usage: npm run cli:batch-transactions -- --operations <file> --fee-payer <wallet> [options]');
            console.log('');
            ...
            console.log('  --delay-between <ms>    Delay between batches in milliseconds (default: 1000)');
        ...
        const operationsFile = path_1.default.resolve(args.operations);
        if (!fs_1.default.existsSync(operationsFile)) {
            console.error(`❌ Error: Operations file not found: ${operationsFile}`);
            process.exit(1);
        }
        // Load fee payer wallet
        const feePayerPath = path_1.default.resolve(args.feePayer);
        if (!fs_1.default.existsSync(feePayerPath)) {
        ...
        console.log('=========================');
        // Load operations
        const operationsData = fs_1.default.readFileSync(operationsFile, 'utf8');
        let operations;
        try {
            ...
            if (parsedData && typeof parsedData === 'object' && !Array.isArray(parsedData)) {
                if (parsedData.operations && Array.isArray(parsedData.operations)) {
                    ...
                    throw new Error('Operations file must contain an array or object with "operations" array');
                }
            }
        ...
        // Load fee payer wallet
        const feePayerData = JSON.parse(fs_1.default.readFileSync(feePayerPath, 'utf8'));
        const feePayer = web3_js_1.Keypair.fromSecretKey(Uint8Array.from(feePayerData));
        ...
        operations = operations.map((op, index) => {
            ...
            if (copy.sender) {
                if (typeof copy.sender === 'string') {
                    ...
                    const walletPath = path_1.default.resolve(copy.sender);
                    ...
                    copy.sender = web3_js_1.Keypair.fromSecretKey(Uint8Array.from(walletData));
                }
            }
            else if (args.wallet) {
                // Fallback: use provided --wallet for ops missing sender
                const fallbackPath = path_1.default.resolve(args.wallet);
                if (!fs_1.default.existsSync(fallbackPath)) {
                    console.error(`❌ Error: Fallback wallet file not found: ${fallbackPath}`);
                    process.exit(1);
                }
                const walletData = JSON.parse(fs_1.default.readFileSync(fallbackPath, 'utf8'));
                copy.sender = web3_js_1.Keypair.fromSecretKey(Uint8Array.from(walletData));
            }
            else {
        ...
        // Validate operations
        const validation = (0, batch_1.validatePumpFunBatchOperations)(operations);
        if (!validation.valid) {
        ...
        const results = await (0, batch_1.batchTransactions)(connection, operations, feePayer, {
            maxParallel,
            delayBetween,
            retryFailed,
...
//# sourceMappingURL=batch-transactions-cli.js.map