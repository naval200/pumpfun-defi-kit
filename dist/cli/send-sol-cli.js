...
const sendSol_js_1 = require("../src/sendSol.js");
const cli_args_js_1 = require("./cli-args.js");
const amounts_js_1 = require("../src/utils/amounts.js");
function showHelp() {
    console.log(`
...
async function main() {
    ...
    if (!args.fromWallet || !args.toAddress || args.amount === undefined) {
        ...
        (0, cli_args_js_1.printUsage)('cli:send-sol');
        return;
    }
    ...
    const amountLamports = (0, amounts_js_1.solToLamports)(amountSol);
    try {
        console.log('💸 SOL Transfer CLI');
        console.log('===================');
        console.log(`Amount: ${(0, amounts_js_1.formatLamportsAsSol)(amountLamports)} SOL`);
        ...
        // Load wallets
        const fromWallet = (0, cli_args_js_1.loadWallet)(args.fromWallet);
        console.log(`👛 Source wallet: ${fromWallet.publicKey.toString()}`);
        let feePayer = undefined;
        if (args.feePayer) {
            feePayer = (0, cli_args_js_1.loadWallet)(args.feePayer);
            console.log(`💸 Fee payer: ${feePayer.publicKey.toString()}`);
        }
        // Validate destination address
        let toAddress;
        try {
            toAddress = new web3_js_1.PublicKey(args.toAddress);
        ...
        // Validate parameters
        const validation = (0, sendSol_js_1.validateSendSolParams)(fromWallet, toAddress, amountLamports);
        ...
        if (balance < requiredBalance) {
            console.log(`❌ Insufficient balance. Need at least ${(0, amounts_js_1.formatLamportsAsSol)(requiredBalance)} SOL`);
            console.log(`💰 Available: ${(0, amounts_js_1.formatLamportsAsSol)(balance)} SOL`);
            console.log(`💡 Required includes: ${(0, amounts_js_1.formatLamportsAsSol)(amountLamports)} SOL + ${(0, amounts_js_1.formatLamportsAsSol)(estimatedFee)} fee + ${(0, amounts_js_1.formatLamportsAsSol)(minRentExemptBalance)} rent exemption`);
            process.exit(1);
        }
        if (args.dryRun) {
            console.log('\n🔍 DRY RUN - Would execute the following:');
            console.log(`  • Send ${(0, amounts_js_1.formatLamportsAsSol)(amountLamports)} SOL from ${fromWallet.publicKey.toString()}`);
        ...
        const result = await (0, sendSol_js_1.sendSol)(connection, fromWallet, toAddress, amountLamports, feePayer);
        if (result.success) {
...
//# sourceMappingURL=send-sol-cli.js.map