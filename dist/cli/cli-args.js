...
exports.loadFeePayerWallet = loadFeePayerWallet;
...
function loadFeePayerWallet(feePayerPath) {
    if (!feePayerPath)
        return null;
    try {
...
function saveTokenInfo(tokenInfo, outputPath) {
    try {
        ...
        (0, debug_1.debugLog)(`💾 Token info saved to ${finalOutputPath}`);
    }
    catch (error) {
        (0, debug_1.logError)(`❌ Failed to save token info: ${error}`);
    }
}
function printUsage(scriptName, options = []) {
