...
const graduation_utils_1 = require("../src/utils/graduation-utils");
...
async function main() {
    ...
    try {
        ...
        // Otherwise load from token info file
            ...
            const tokenInfoPath = args.inputToken || path_1.default.join(process.cwd(), 'token-info.json');
            if (!fs_1.default.existsSync(tokenInfoPath)) {
                console.log(`❌ Token info file not found: ${tokenInfoPath}`);
                console.log('💡 Please provide --mint or create a token first with: npm run cli:bond-create-token');
                return;
            }
        ...
        const isGraduated = await (0, graduation_utils_1.checkGraduationStatus)(connection, tokenMint);
        ...
        const analysis = await (0, graduation_utils_1.getGraduationAnalysis)(connection, tokenMint);
...
//# sourceMappingURL=graduation-check-cli.js.map