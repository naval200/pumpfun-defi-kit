...
export async function buyTokens(
  ...
  try {
    ...
    debugLog(`  🔢 Raw Base Amount: ${poolBaseAmount.toString()} (hex: 0x${poolBaseAmount.toString(16)})`);
    debugLog(`  🔢 Raw Quote Amount: ${poolQuoteAmount.toString()} (hex: 0x${poolQuoteAmount.toString(16)})`);
    debugLog(`  📈 Pool Value: ${((baseReserve * (quoteReserve / 1e9)) / baseReserve).toFixed(9)} SOL per TBC token`);
    ...
    debugLog(`  📊 Current Quote Reserve: ${quoteReserve.toLocaleString()} lamports (${(quoteReserve / 1e9).toFixed(9)} SOL)`);
    debugLog(`  📊 New Quote Reserve: ${newQuoteReserve.toLocaleString()} lamports (${(newQuoteReserve / 1e9).toFixed(9)} SOL)`);
    ...
    debugLog(`  📈 Price Impact: ${((newQuoteReserve - quoteReserve) / quoteReserve * 100).toFixed(4)}%`);
    ...
    try {
      const safeState = JSON.parse(JSON.stringify(effectiveState, (key, value) =>
      ...
      if (safeState.poolBaseAmount && safeState.poolQuoteAmount) {
        ...
        debugLog(`  📊 Pool Quote Amount: ${(Number(safeState.poolQuoteAmount) / 1e9).toFixed(9)} SOL`);
    ...
    instructions.forEach((ix, i) => {
      ...
      if (ix.data.length > 0) {
        const dataHex = Array.from(ix.data.slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join('');
        debugLog(`  🔢 Data: 0x${dataHex}`);
      }
      
      // Enhanced AMM instruction analysis
      if (i === 3) { // AMM instruction is typically the 4th one (after ATA creation, SOL transfer, token init)
        ...
        const keyAccounts = [
          { index: 0, name: 'Pool Key', desc: 'Pool identifier' },
          { index: 1, name: 'User', desc: 'Buyer wallet' },
          { index: 4, name: 'Base Mint', desc: 'TBC token mint' },
          { index: 5, name: 'Quote Mint', desc: 'SOL mint' },
          { index: 6, name: 'User Base ATA', desc: 'User TBC account' },
          { index: 7, name: 'User Quote ATA', desc: 'User SOL account' },
          { index: 8, name: 'Pool Base ATA', desc: 'Pool TBC account' },
          { index: 9, name: 'Pool Quote ATA', desc: 'Pool SOL account' }
        ];
        
        keyAccounts.forEach(({ index, name, desc }) => {
          if (ix.keys[index]) {
            const key = ix.keys[index];
            debugLog(`      ${index.toString().padStart(2)}: ${name} = ${key.pubkey.toString()}`);
            debugLog(`           ${key.isSigner ? '👤 Signer' : '👁️  Non-signer'}, ${key.isWritable ? '✏️  Writable' : '👁️  Read-only'}`);
          }
        });
      }
    });
    ...
    if (options?.assumeAccountsExist) {
      ...
      const afterCount = instructions.length;
      debugLog(`  📊 Filtered instructions: ${beforeCount} → ${afterCount} (removed ${beforeCount - afterCount} ATA creation)`);
    }
...
export async function createSignedAmmBuyTransaction(
  ...
  try {
    ...
    // Convert SOL amount to lamports before creating BN
    const quoteAmountLamports = Math.floor(quoteAmount * 1e9);
    debugLog(`  🔢 Unit Conversion: ${quoteAmount} SOL → ${quoteAmountLamports.toLocaleString()} lamports`);
    ...
    if (options?.assumeAccountsExist) {
      debugLog(`  ⛔ Assuming accounts exist: filtering ATA creation instructions`);
      const beforeCount = instructions.length;
      instructions = instructions.filter(
        ix => ix.programId.toString() !== ASSOCIATED_TOKEN_PROGRAM_ID.toString()
      );
      const afterCount = instructions.length;
      debugLog(`    📊 Filtered: ${beforeCount} → ${afterCount} instructions`);
    }
    ...
    if (feePayer && feePayer.publicKey.toString() !== wallet.publicKey.toString()) {
      ...
      debugLog(`    ✍️  Signed by: ${wallet.publicKey.toString()} + ${feePayer.publicKey.toString()}`);
    } else {
