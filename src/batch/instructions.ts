  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
    ...
    try {
      ...
      if (feePayer) {
        ...
        const feePayerAlreadyInSigners = signersInOrder.some(s =>
          s.publicKey.equals(feePayer.publicKey)
        );
      ...
      debugLog(
        `🔍 Batch ${batchIndex + 1}: ${allInstructions.length} instructions, ${signersInOrder.length} signers`
      );
