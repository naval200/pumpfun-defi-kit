...
import { PUMP_PROGRAM_ID } from '../../src/bonding-curve/idl/constants';
...
async function checkCreatorAccountStatus(
  ...
  try {
    ...
    const [userVolumeAccumulator] = PublicKey.findProgramAddressSync(
      ['user-volume-accumulator', creator.toBuffer()],
      PUMP_PROGRAM_ID
    );
    ...
    const [creatorVaultExists, globalVolumeAccumulatorExists, userVolumeAccumulatorExists, eventAuthorityExists] = await Promise.all([
      connection.getAccountInfo(creatorVault).then(info => info !== null).catch(() => false),
      connection.getAccountInfo(globalVolumeAccumulator).then(info => info !== null).catch(() => false),
      connection.getAccountInfo(userVolumeAccumulator).then(info => info !== null).catch(() => false),
      connection.getAccountInfo(eventAuthority).then(info => info !== null).catch(() => false)
    ]);
    ...
    return {
      onboarded: false,
      missingAccounts: ['creatorVault', 'globalVolumeAccumulator', 'userVolumeAccumulator', 'eventAuthority'],
      details: {
        creatorVault: false,
...
async function main() {
  ...
  try {
    ...
    const status = await checkCreatorAccountStatus(connection, wallet.publicKey, mint);
    ...
    if (status.onboarded) {
      ...
      logSuccess('🚀 You can still proceed to buy initial tokens - PDAs will be created automatically!');
    }
