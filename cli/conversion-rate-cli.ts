...
import { parseArgs, loadTokenInfo, printUsage } from './cli-args';
import { getTokenToSolConversionRate, getSolToTokenConversionRate } from '../src/utils/conversion-rate';
...
async function main() {
  ...
  const direction = (args.direction as string) || 'token-to-sol';
  const showBoth = args.both || false;
  const poolKey = args.poolKey ? new PublicKey(args.poolKey) : undefined;
  ...
  try {
    ...
    if (showTokenToSol) {
      ...
      if (rate !== null) {
        ...
        console.log(`   ${tokenAmount} token(s) = ${formatLamportsAsSol(solAmount * 1e9)} SOL`);
